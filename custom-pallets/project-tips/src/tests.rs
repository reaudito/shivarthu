...
fn schelling_game_incentives_test() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ProjectTips::add_incentive_count(RuntimeOrigin::signed(14), 1));
		...
		assert_ok!(ProjectTips::add_incentive_count(RuntimeOrigin::signed(15), 1));
...
fn full_schelling_game_func(who_ask_tipper: u64, start_block_number: u64) {
	...
	for j in 4..30 {
		assert_ok!(ProjectTips::apply_jurors(RuntimeOrigin::signed(j), project_id, j * 100));
	}
	...
	assert_ok!(ProjectTips::pass_period(RuntimeOrigin::signed(5), project_id));

	assert_ok!(ProjectTips::draw_jurors(RuntimeOrigin::signed(5), project_id, 5));
	...
	let hash = sp_io::hashing::keccak_256("1salt".as_bytes());
	assert_ok!(ProjectTips::commit_vote(RuntimeOrigin::signed(4), project_id, hash));
	...
	let hash = sp_io::hashing::keccak_256("1salt3".as_bytes());
	assert_ok!(ProjectTips::commit_vote(RuntimeOrigin::signed(7), project_id, hash));

	let hash = sp_io::hashing::keccak_256("1salt4".as_bytes());
	assert_ok!(ProjectTips::commit_vote(RuntimeOrigin::signed(13), project_id, hash));

	let hash = sp_io::hashing::keccak_256("1salt5".as_bytes());
	assert_ok!(ProjectTips::commit_vote(RuntimeOrigin::signed(14), project_id, hash));

	let hash = sp_io::hashing::keccak_256("0salt6".as_bytes());
	assert_ok!(ProjectTips::commit_vote(RuntimeOrigin::signed(15), project_id, hash));
	...
	assert_ok!(ProjectTips::add_incentive_count(RuntimeOrigin::signed(14), project_id));
	assert_ok!(ProjectTips::add_incentive_count(RuntimeOrigin::signed(15), project_id));
}
...
fn get_tip_test() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ProjectTips::release_tip(RuntimeOrigin::signed(14), project_id));
