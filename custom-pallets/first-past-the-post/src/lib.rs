...
pub mod pallet {
	...
	// Storage to track votes for each candidate
	#[pallet::storage]
	#[pallet::getter(fn votes)]
	pub type Votes<T: Config> = StorageMap<_, Twox64Concat, T::AccountId, u32, ValueQuery>;

	// Storage to track if a voter has already voted
	#[pallet::storage]
	#[pallet::getter(fn has_voted)]
	pub type HasVoted<T: Config> = StorageMap<_, Twox64Concat, T::AccountId, bool, ValueQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn vote(origin: OriginFor<T>, candidate: T::AccountId) -> DispatchResult {
			...
			ensure!(!HasVoted::<T>::get(&who), Error::<T>::AlreadyVoted);
